...
class gaussianProcess():
    ...
    def exponential_quadratic_kernel(selfx, xn, xm, param):
        ...
        s = param[2]
        # I don't no dot will happen error
        t = param[3]*np.multiply.outer(xn, xm)
        return f+s+t
    ...
    def plot_gp(self, train_X, train_y, x, y, y1, y2, title, save=0):
        ...
        plt.scatter(train_X, train_y, facecolors='none',
                    edgecolors='b')  # plot the data point
        plt.title(str(title))
        #plt.xlim(0, 2)
        #plt.ylim(-10, 15)
        plt.xlabel('x')
    ...
    def process(self, X, y, iter=5000, split=50):
        ...
        for p in self.parm:
            C_inv = np.linalg.inv(self.exponential_quadratic_kernel(
                train_X, train_X, p) + self.beta_inv * np.identity(split))

            for i in range(iter):
                k = self.exponential_quadratic_kernel(train_X, x[i], p)
                c = self.exponential_quadratic_kernel(
                    x[i], x[i], p) + self.beta_inv
                # mean
        ...
        # ARD
        print('ARD')
        parameters_ard = [[32, 32, 32, 32]]
        params_ard = self.ARD(train_X, train_y, parameters_ard)
        C_inv_ard = np.linalg.inv(self.exponential_quadratic_kernel(
            train_X, train_X, params_ard[-1]) + self.beta_inv * np.identity(split))
        for i in range(iter):
            k = self.exponential_quadratic_kernel(
                train_X, x[i], params_ard[-1])
            c = self.exponential_quadratic_kernel(
                x[i], x[i], params_ard[-1]) + self.beta_inv
            # mean
...
def PCA(features, n=2, n_eigVect=None, typ=1):
    ...
    # transform to low dim data (same as the return of sklearn fit_transform())
    Trans = center.dot(n_eigVect)
...
class supportVectorMachine():
    ...
    def fit(self, X, y, vs='ovo', c=1):
        '''
        using sklearn to fit linear or poly kernel
        '''
        if self.type_ == 'linear':
            clf = SVC(kernel=self.type_, C=c, decision_function_shape=vs)
        else:
            clf = SVC(kernel=self.type_, C=c, degree=2,
                      decision_function_shape=vs)
    ...
    def kernel_function(self, xn, xm):
        if self.type_ == 'linear':
            return self.linear_phi(xn).dot(self.linear_phi(xm).T)
        else:
            return self.poly_phi(xn).dot(self.poly_phi(xm).T)

    def prepare_parameter(self, X, size=100):
        # target
        target_dict = {}
        target_dict[(0, 1)] = np.concatenate(
            (np.ones(size), np.full([size], -1), np.zeros(size)))
        target_dict[(0, 2)] = np.concatenate(
            (np.ones(size), np.zeros(size), np.full([size], -1)))
        target_dict[(1, 2)] = np.concatenate(
            (np.zeros(size), np.ones(size), np.full([size], -1)))
        # multiplier
        multiplier = np.zeros([len(X), 2])
        multiplier[self.sv_index] = self.coef.T

        multiplier_dict = {}
        multiplier_dict[(0, 1)] = np.concatenate(
            (multiplier[:size * 2, 0], np.zeros(size)))
        multiplier_dict[(0, 2)] = np.concatenate(
            (multiplier[:size, 1], np.zeros(size), multiplier[size * 2:, 0]))
        multiplier_dict[(1, 2)] = np.concatenate(
            (np.zeros(size), multiplier[size:, 1]))
        return target_dict, multiplier_dict

    def get_w_b(self, a, t, x):
        # PRML 7.29, 7.37
        if self.type_ == 'linear':
            w = (a*t).dot(self.linear_phi(x))
        else:
            w = (a*t).dot(self.poly_phi(x))
        ...
        if Nm == 0:
            ...
            # PRML 7.18 7.37
            b = np.mean(
                t[M_indexes] - (a*t)[S_indexes].dot(self.kernel_function(x[M_indexes], x[S_indexes]).T))

        return w, b

    def train(self, X):
        target_dict, multiplier_dict = self.prepare_parameter(
            X)
        w_dict = {}
        b_dict = {}
        ...
        for c1, c2 in self.class_label:
            w, b = self.get_w_b(
                multiplier_dict[(c1, c2)], target_dict[(c1, c2)], X)
                
            w_dict[(c1, c2)] = w
            b_dict[(c1, c2)] = b
        return w_dict, b_dict

    def predict(self, X, w_dict, b_dict):
        pred = []
        for index in range(len(X)):
            ...
            for c1, c2 in self.class_label:
                ...
                b = b_dict[(c1, c2)]
                if self.type_ == 'linear':
                    y = w.dot(self.linear_phi(X[index]).T) + b
                else:
                    y = w.dot(self.poly_phi(X[index]).T) + b
                if y > 0:
                    votes += [c1]
                else:
                    votes += [c2]
    ...
    def plot(self, X, t, xx, yy, prediction):
        ...
        plt.scatter(X[class0_indexes][:, 0], X[class0_indexes]
                    [:, 1], c='b', marker='x', label="class 0")
        plt.scatter(X[class1_indexes][:, 0], X[class1_indexes]
                    [:, 1], c='g', marker='*', label="class 1")
        plt.scatter(X[class2_indexes][:, 0], X[class2_indexes]
                    [:, 1], c='r', marker='o', label="class 2")
        plt.legend()
    ...
    def process(self, X_csv='dataset/x_train.csv', T_csv='dataset/t_train.csv'):
        ...
        linear_svm.fit(X, y)
        w_dict, b_dict = linear_svm.train(X)
        ...
        poly_svm.fit(X, y)
        w_dict, b_dict = poly_svm.train(X)
...
class kmeans():
    def __init__(self, data, k=2, max_iter=300):
        ...
        # else 0
        # rn_k size (x*y, k)
        self.rn_k = np.ones([len(data), self.k])

    def minimize_j(self, data):
        for i in range(self.max_iter):
            distance = np.sum((data[:, None] - self.mu)**2, axis=2)
            # pixel -> pixels*k
            rn_k = self.eye[np.argmin(distance, axis=1)]

            if np.array_equal(rn_k, self.rn_k):
                break
            else:
                self.rn_k = rn_k
            # mu_k = sum(rn_k*Xn)/sum(rn_k)
            self.mu = np.sum(rn_k[:, :, None] * data[:, None],
                             axis=0) / np.sum(rn_k, axis=0)[:, None]

    def rgb_table(self, _type):
        tb = pt.PrettyTable()
    ...
    def generate_image(self, _type):
        if _type == 'K_means':
            new_data = (self.mu[np.where(self.rn_k == 1)[1]] * 255).astype(int)
        else:
            new_data = (
                self.mu[np.argmax(self.gaussians, axis=0)] * 255).astype(int)

        disp = Image.fromarray(new_data.reshape(
            height, width, depth).astype('uint8'))
...
class gaussianMixtureModel():
    def __init__(self, k, k_mean_rn_k, k_mean_mu, data, lr=0.01, max_iter=100):
        ...
        # k *rgb*rgb
        self.cov = np.array(
            [np.cov(data[np.where(k_mean_rn_k[:, k] == 1)[0]].T) for k in range(self.k)])
        self.lr = lr
